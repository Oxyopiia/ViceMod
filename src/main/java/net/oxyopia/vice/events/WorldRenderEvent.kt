...
open class WorldRenderEvent(val matrices: MatrixStack, val tickDelta: Float, val camera: Camera, val gameRenderer: GameRenderer, val projectionMatrix: Matrix4f) : ViceEvent() {
	...
	class AfterEntities(matrices: MatrixStack, tickDelta: Float, camera: Camera, gameRenderer: GameRenderer, projectionMatrix: Matrix4f) : WorldRenderEvent(matrices, tickDelta, camera, gameRenderer, projectionMatrix)
}