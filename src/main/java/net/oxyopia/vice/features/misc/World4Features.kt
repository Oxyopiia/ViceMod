...
object World4Features {
	...
	fun onHudRender(event: RenderInGameHudEvent) {
		...
		if (Vice.config.DEVMODE) {
			xPos = (event.scaledWidth / 2 - 1) + Vice.devConfig.COOKING_ORDER_HUD_X_OFFSET_LOCATION
			yPos = (event.scaledHeight / 2 - 1) + Vice.devConfig.COOKING_ORDER_HUD_Y_OFFSET_LOCATION
		}
